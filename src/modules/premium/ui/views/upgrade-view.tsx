...
import { PricingCard } from "../components/pricing-cagd"
...
export const UpgradeView = () => {
    ...
    const { data: currentSubsscription } = useSuspenseQuery(
            ...
            <div className=" mt-4 flex-1 flex flex-col gap-y-10 items-center">
                <h5 className="font-medium text-2xl md:text-3xl">
                    ...
                    <span className="font-semibold text-primary">
                        {currentSubsscription?.name ?? "Free"}
                    </span>{" "}
                    plan
                </h5>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {products.map((product) => {
                        const isCurrentProduct = currentSubsscription?.id === product.id;
                        const isPremium = !!currentSubsscription;
...
export const UpgradeViewLoading = () => {
    return (
        <LoadingState title="Loading" description="This nay take a few seconds..." />
    )
}

export const UpgradeViewError = () => {
    return (
        <ErrorState title="Error" description="Please try  later" />
    )
}
