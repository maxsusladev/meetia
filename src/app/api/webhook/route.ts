...
export async function POST(req: NextRequest) {
    ...
    if (eventType === "call.session_started") {
        ...
        if (!meetingId) {
            return NextResponse.json({ error: "Mising meetingId" }, { status: 4000 })
        }
        ...
        if (messageId && processedMessageIds.has(messageId)) {
            console.log("DEV DEDUPE HIT:", messageId);
        ...
        if (senderId === existingAgent.id) {
            console.log("ignore agent message")
            return NextResponse.json({ ok: true })
        }
        ...
        if (senderId !== existingAgent.id) {
            ...
            if (!GPTResponseText) {
                return NextResponse.json({ error: "No response frop GPT" }, { status: 400 })
            }
